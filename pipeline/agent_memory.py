...
Структура:
  {
    "kv": {                         ← произвольные ключи-значения
      "agent_status_updated_at": { "EDITOR": "2025-03-28T12:00:00", ... },
...
class AgentMemory:
    ...
    def set_agent_status(self, agent_name: str, status: str) -> None:
        """Обновляет статус агента и сохраняет на диск (для ContentHub / снимок после краша процесса)."""
        key_ts = "agent_status_updated_at"
        with self._lock:
            self._agents[agent_name] = status
            meta = self._kv.get(key_ts)
            if not isinstance(meta, dict):
                meta = {}
            meta[agent_name.upper()] = datetime.now().isoformat(timespec="seconds")
            self._kv[key_ts] = meta
            self._save()
