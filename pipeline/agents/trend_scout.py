...
Алгоритм:
  1. Опрашивает источники: Google Trends, VK Video, RuTube, OK
...
Конфиг (через .env):
  ...
  TREND_SCOUT_SOURCES       = google,vk,rutube,ok — какие источники использовать
"""
...
class TrendScout(BaseAgent):
    ...
    def _collect_trends(self) -> None:
        ...
        try:
            ...
            # Используем тот же LRU/контекст ротации, что и у SCOUT
            with scout_pipeline_cycle_account(self.memory):
                sources_str = str(getattr(cfg, "TREND_SCOUT_SOURCES", "google,vk,rutube,ok"))
                raw_sources = [s.strip().lower() for s in sources_str.split(",") if s.strip()]
                # Backward compatibility: старые имена источников приводим к новым.
                source_aliases = {
                    "youtube": "vk",
                    "yt": "vk",
                    "tiktok": "ok",
                ...
                sources = [source_aliases.get(s, s) for s in raw_sources]
    ...
    def _fetch_sources(
        ...
        if "vk" in sources:
            try:
                kws = fetch_vk_video_trends()
                output.extend(kws)
                logger.debug("[TREND_SCOUT] VK: %d", len(kws))
            except Exception as exc:
                logger.warning("[TREND_SCOUT] VK Trends ошибка: %s", exc)

        if "rutube" in sources:
            try:
                kws = fetch_rutube_trends()
                output.extend(kws)
                logger.debug("[TREND_SCOUT] RuTube: %d", len(kws))
            except Exception as exc:
                logger.warning("[TREND_SCOUT] RuTube Trends ошибка: %s", exc)

        if "ok" in sources:
            try:
                kws = fetch_ok_trends()
                output.extend(kws)
                logger.debug("[TREND_SCOUT] OK: %d", len(kws))
            except Exception as exc:
                logger.warning("[TREND_SCOUT] OK Trends ошибка: %s", exc)
