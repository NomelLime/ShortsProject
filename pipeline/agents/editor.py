...
def _target_platform_from_account(acc_cfg: Optional[dict]) -> str:
    ...
    valid_platforms = {"vk", "rutube", "ok"}
    fallback = "vk"
    if not acc_cfg:
        return fallback
    plats = acc_cfg.get("platforms", [fallback])
    if isinstance(plats, str):
        plats = [plats]
    p = (plats[0] if plats else fallback).lower()

    try:
        from pipeline import config as _cfg
        aliases = getattr(_cfg, "PLATFORM_ALIASES", {}) or {}
        p = str(aliases.get(p, p)).lower()
    except Exception:
        pass
...
def _sanitize_llm_input(text: str, max_len: int = _MAX_PROMPT_FIELD_LEN) -> str:
    ...
    Защита от prompt injection: заголовки из внешних источников могут содержать
