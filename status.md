...
| Компонент | Описание |
|-----------|----------|
| **`../shared_gpu_lock/`** | Устанавливается editable: `-e ../shared_gpu_lock` в `requirements.txt` ShortsProject и Orchestrator. Пакет **`shared-gpu-lock`**: модуль **`file_lock`** (`acquire_gpu_lock`, `get_gpu_lock_file_path`) и **`gpu_manager`** (`GPUResourceManager`, `GPUPriority`, `get_gpu_manager`). Зависимость: **`portalocker>=2.8`**. |
| **`pipeline/agents/gpu_manager.py`** | Реэкспорт из `shared_gpu_lock.gpu_manager` — внешние импорты `from pipeline.agents.gpu_manager import …` без изменений. |
| **`pipeline/shared_gpu_lock.py`** | Реэкспорт из `shared_gpu_lock.file_lock` — совместимость с `activity_vl`, `niche`, тестами. |
| **`GPUResourceManager.acquire()`** | После выдачи in-process слота (очередь + семафор) перед телом работы вызывается **`acquire_gpu_lock`** на тот же файл, что и прямые Ollama-вызовы вне менеджера. В одном потоке lock **реентерабелен** (вложенные `acquire_gpu_lock` + `acquire` не дублируют файловый lock). |
| Параметры API | `use_cross_process_lock=True`, `cross_process_lock_timeout=360`. В **`TestGPUManager`** — `use_cross_process_lock=False`, чтобы не трогать диск. |
| Путь к lock по умолчанию | **`GitHub/.gpu_lock`** (рядом с `ShortsProject` / `Orchestrator`), иначе `%TEMP%\shared_gpu_lock\gpu.lock`. Переопределение: **`SHARED_GPU_LOCK_PATH`**. |

**Тесты:** `pytest tests/test_agents.py::TestGPUManager`, `tests/test_orchestrator.py` — ✅

#### Code review — SGL-01 … SGL-05 (`shared_gpu_lock`)

| ID | Исправление |
|----|-------------|
| **SGL-01** | **`get_gpu_manager()`**: double-checked locking с **`threading.Lock()`**, исключена гонка при первом обращении из нескольких потоков. |
| **SGL-02** | **`_dispatcher`**: при исчерпании ретраев — **`task.aborted = True`** + **`task.event.set()`**; в **`acquire()`** при **`aborted`** — **`RuntimeError`** без **`semaphore.release()`** (слот диспетчером не выдан). Параметр **`dispatcher_semaphore_acquire_timeout`** (по умолчанию 300 с) для тестов с коротким таймаутом. |
| **SGL-03** | **`acquire()`**: один **`finally`** с **`semaphore.release()`** после успешного ожидания слота; при **`TimeoutError`** file-lock — снятие **`active`** в **`except`**, ровно одно освобождение семафора. |
| **SGL-04** | В **`shared_gpu_lock/tests/`**: приоритет/лимит параллелизма, реентерабельность **`acquire_gpu_lock`**, таймаут lock при удержании другим потоком, синглтон **`get_gpu_manager`**, сценарий **aborted** диспетчера. **`python -m pytest tests`** в каталоге пакета. |
| **SGL-05** | **`shared_gpu_lock/.gitignore`**: `__pycache__/`, `*.pyc`, артефакты сборки. |

---

### Сессия 17 (28.03.2026) — `send_telegram`: дедупликация только после успешной доставки + исправление «ложного дубля»

| Проблема | Исправление |
|----------|-------------|
| Запись в **`_tg_dedup_cache`** выполнялась **до** **`requests.post()`**. При **`ConnectionError`** повтор с тем же текстом шёл по ветке «дубль» и возвращал **`True`** без HTTP — ломался **`test_returns_false_on_request_error`**. | Timestamp в кеш пишется **только после HTTP 200**; при ошибке/не-200 кеш не трогаем; не 200 → **`return False`**. |
| **`_tg_dedup_cache.get(msg_hash, 0.0)`** для отсутствующего ключа: дефолт **`0.0`** + **`time.monotonic()`** в первые ~**300 с** после старта процесса давали **`now - 0 < _tg_dedup_window`** → **любое** новое сообщение считалось дублем; критические алерты (CAPTCHA, бан и т.д.) **не уходили в Telegram** в первые 5 минут. | **`last_for_msg = _tg_dedup_cache.get(msg_hash)`** без дефолта; дедуп только если **`last_for_msg is not None`** (ключ появился после реальной успешной отправки). |

**Тесты:** `pytest tests/test_pipeline.py::TestSendTelegram` — **3/3** ✅
...
| Область | Изменение |
|------|-----------|
| `pipeline/content_locale.py` | Новый модуль резолва локали контента по GEO/Accept-Language/правилам; экспорт `get_proxy_country` для единого источника GEO в пайплайне. |
| `pipeline/ai.py` | Прокинут `content_locale` в prompts и cache-ключи, добавлены fallbacks и хелперы для стабильной генерации при отсутствующих данных по языку. |
| `pipeline/agents/editor.py` + `pipeline/agents/visionary.py` | Интеграция `content_locale` в агентные шаги подготовки/обработки контента. |
| `pipeline/slicer.py` + `setup_account.py` | Протянуты данные локали в связанные этапы, чтобы итоговый контент и публикация оставались согласованы по языку. |
| `tests/*` | Добавлены/обновлены тесты на `content_locale` и связанные флоу, чтобы зафиксировать поведение fallback-цепочек. |

**Интеграция с PreLend в рамках сессии:**
- Для оффер-шаблонов в PreLend добавлен i18n fallback `en` и локализованы offer-шаблоны.
- Ветка `templates/cloaked/*` намеренно не трогалась.
...
GPU Manager гарантирует: только одна тяжёлая задача (LLM / TTS / VideoGen / Encode) одновременно **в процессе**; между процессами — общий файловый lock пакета **`shared_gpu_lock`** (см. **Сессию 18**).
...
| `pipeline/agents/gpu_manager.py` | Реэкспорт **`shared_gpu_lock.gpu_manager`**. Priority queue + кросс-процессный lock (с **Сессией 18**). `GPUPriority`: CRITICAL / LLM / TTS / VIDEO_GEN / ENCODE. Context manager + decorator |
...
| Класс | Тесты |
|-------|-------|
| `TestGPUManager` | priority_order, concurrent_limit=1, stats_tracking, decorator_usage; `use_cross_process_lock=False` (**Сессия 18**) |
...
| `pipeline/shared_gpu_lock.py` | Реэкспорт **`shared_gpu_lock.file_lock`**; кросс-процессный GPU-lock (`portalocker`), тот же путь к файлу, что у Orchestrator (см. **Сессию 18**) |
...
| `requirements.txt` | + `portalocker` (кросс-процессные GPU-блокировки); позже — **Сессия 18**: `-e ../shared_gpu_lock` |

#### Координация GPU

Три уровня защиты от конкуренции:
1. `_vl_semaphore` — не более `ACTIVITY_VL_CONCURRENCY=2` одновременных activity-сессий
2. **`shared_gpu_lock`** (пакет + реэкспорт в `pipeline/shared_gpu_lock.py`) — сериализация Ollama-вызовов между процессами; **`GPUResourceManager`** также берёт этот lock внутри `acquire()` (с **Сессией 18**)
...
- VISIONARY → AI метаданные через гибридный VL backend: `ollama` или `transformers` (Qwen2.5-VL), с безопасным fallback и anti-generic регенерацией
...
- Telegram → rate limiting, дедупликация (кеш только после HTTP 200; проверка дубля только при наличии ключа — без ложного «дубля» в первые 5 мин после старта процесса, Сессия 17), whitelist, `critical` фильтр (Сессия 4)
...
Рядом с репозиторием (каталог **`GitHub/shared_gpu_lock/`**): пакет **`shared-gpu-lock`** (`pip install -e ../shared_gpu_lock`) — общий **`file_lock`** + **`gpu_manager`**.
...
│   ├── notifications.py              ← Telegram: rate limit; дедуп после 200 + ключ в кеше для cooldown
│   ├── activity_vl.py                ← VL-эмуляция активности (полный объект аккаунта)
│   ├── shared_gpu_lock.py            ← реэкспорт shared_gpu_lock.file_lock (portalocker, ORC)
...
│       ├── gpu_manager.py            ← реэкспорт shared_gpu_lock.gpu_manager (+ file-lock в acquire)
...
| `pipeline/agents/gpu_manager.py` | `PriorityQueue` без лимита | + `maxsize=100` (логика в пакете **`shared_gpu_lock`**, см. **Сессию 18**) |
| `pipeline/shared_gpu_lock.py` | Хардкод пути к `.gpu_lock` | `_cfg.BASE_DIR.parent / ".gpu_lock"` → позже унифицировано в **`get_gpu_lock_file_path()`** (**Сессия 18**) |
...
| R12-5 | ✅ верифицирован | `pipeline/shared_gpu_lock.py` → пакет **`shared_gpu_lock`** | FIX#V3-2: `raise TimeoutError` уже применён (логика в **`shared_gpu_lock.file_lock`**, см. **Сессию 18**) |
...
### Сессия 14 (23.03.2026) — Улучшение качества метаданных: transformers+images, caption-per-frame, anti-generic

Фокус сессии: сделать метаданные содержательными даже при нестабильных ответах LLM и убрать зависимость от имени файла.

#### Что изменено

| Файл | Что изменилось |
|------|---------------|
| `pipeline/config.py` | Добавлены параметры VL backend: `VL_BACKEND` (`ollama`/`transformers`), `QWEN_VL_MODEL_ID`, `QWEN_VL_VIDEO_FPS`, `QWEN_VL_MAX_PIXELS`, `QWEN_VL_MAX_NEW_TOKENS`, `QWEN_VL_TEMPERATURE`, `QWEN_VL_FLASH_ATTN`. |
| `pipeline/ai.py` | Введён гибридный слой генерации `_video_vl_generate_with_fallback`: сначала `transformers` (Qwen2.5-VL), при ошибке — fallback в Ollama. |
| `pipeline/ai.py` | Добавлен рабочий `transformers+images` путь (`_hf_qwen_images_generate`) — хронологический набор ключевых кадров без `qwen-vl-utils` video-reader зависимости. |
| `pipeline/ai.py` | Обязательный `caption-per-frame` (`_caption_keyframes_vl`) и детерминированный seed (`_build_deterministic_meta_seed`) для `title/description`. |
| `pipeline/ai.py` | Жёсткая браковка общих ответов (`_variant_too_generic`) и автоматическая перегенерация в цикле попыток. |
| `pipeline/ai.py` | Расширены anti-template фильтры (`_GENERIC_META_PATTERNS`), включая служебные и слишком общие фразы. |
| `pipeline/ai.py` | Улучшены fallback/salvage: при слабом ответе формируются осмысленные поля из captions/контекста/транскрипта, а не из имени файла. |

#### Проверка совместимости

- Smoke-импорт: `pipeline.ai`, `pipeline.main_processing`, `pipeline.slicer`, `pipeline.agents.editor`, `pipeline.agents.visionary` — успешно.
- Фокусные тесты: `TestAiMetadataQuality` + `TestStageSlice` — `10 passed`.
- Линтер по изменённым файлам (`pipeline/ai.py`, `pipeline/config.py`) — без ошибок.

#### Итог поведения

- Логика пайплайна не ломается: при проблемах нового backend автоматически сохраняется рабочий путь через Ollama.
- При успешном `transformers+images` качество метаданных выше за счёт хронологического анализа ключевых кадров и caption-per-frame.
