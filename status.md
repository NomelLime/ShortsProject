...
### Сессия 17 (28.03.2026) — `send_telegram`: дедупликация только после успешной доставки

| Проблема | Исправление |
|----------|-------------|
| В **`pipeline/notifications.py`** запись в **`_tg_dedup_cache`** выполнялась **до** **`requests.post()`**. При **`ConnectionError`** (или другой ошибке) сообщение всё равно считалось «уже отправленным»; повтор с тем же текстом шёл по ветке «дубль» и возвращал **`True`** без HTTP-запроса — тест **`test_returns_false_on_request_error`** мог стабильно или в зависимости от порядка тестов давать **`True` вместо `False`**. |
| В кеш дедупа пишется timestamp **только после HTTP 200**; при исключении или не-200 кеш не обновляем (повторная попытка возможна). Ответ API не 200 → явный **`return False`**. |
...
- Telegram → rate limiting, дедупликация (запись в кеш только после HTTP 200, Сессия 17), whitelist, `critical` фильтр (Сессия 4)
...
│   ├── notifications.py              ← Telegram: rate limit + дедуп после успешной отправки
